...
public:
  ...
  const std::vector<torch::Tensor> &keypoints() const { return keypoints_; }

  const std::vector<std::vector<Eigen::Vector3d>> &bearings() const {
    return bearings_;
  }

  const std::vector<torch::Tensor> &descriptors() const { return descriptors_; }
  ...
  cv::Mat drawRawImage(const int &cam_id = -1);

  cv::Mat drawKeyPoint(const int &cam_id = -1);

  cv::Mat drawMatchedKeyPoint(const int &cam_id = -1);

  cv::Mat drawReprojKeyPoint(const int &cam_id = -1);
...
private:
  ...
  std::vector<torch::Tensor> descriptors_;
...
private:
  ...
  //   ar(cereal::make_nvp("id", id_), cereal::make_nvp("keypoints",
  //   keypoints_),

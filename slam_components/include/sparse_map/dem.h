...
struct DEM {
    DEM() = default;
    DEM(const double& _grid_resolution, const std::vector<Eigen::Vector3d>& _pointcloud) {
        ...
        data.resize(grid_height, std::vector<double>(grid_width, std::numeric_limits<double>::lowest()));
    ...
    void save2(const std::string& filename) {
        std::cout << "Saving DEM to " << filename << std::endl;
    ...
    void print() {
        std::cout << std::fixed << "Grid resolution: " << grid_resolution << std::endl;
        std::cout << std::fixed << "MinX: " << minX << " MinY: " << minY << std::endl;
        std::cout << std::fixed << "Nodata value: " << nodata_value << std::endl; 
        std::cout << std::fixed << "Data(" << data.size() << "," << data[0].size() << ")" << std::endl;
    }

...
std::vector<torch::Tensor> corr_index_cuda_forward(torch::Tensor volume,
  ...
  const dim3 blocks((wd + BLOCK - 1) / BLOCK, (ht + BLOCK - 1) / BLOCK,
                    batch_size);
  ...
  torch::Tensor corr =
      torch::zeros({batch_size, 2 * radius + 1, 2 * radius + 1, ht, wd}, opts);

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      volume.type(), "sampler_forward_kernel", ([&] {
...
std::vector<torch::Tensor> corr_index_cuda_backward(torch::Tensor volume,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      volume.type(), "sampler_backward_kernel", ([&] {
...
corr_index_forward(torch::Tensor volume, torch::Tensor coords, int radius) {
  CHECK_INPUT(volume);
  CHECK_INPUT(coords);
